CREATE OR REPLACE VIEW `austin_choi_omni_agent_catalog`.`agent_improvement_loop`.`mv_token_efficiency_and_tool_call_redundancy_metric_view`
WITH METRICS
LANGUAGE YAML
AS $$
version: '1.1'
source: austin_choi_omni_agent_catalog.agent_improvement_loop.l0_session_metrics
comment: 'Auto-generated (Stage 6) from L3/RLM recommendation rank 2 (recurs across 7 trace(s)): Token efficiency and tool-call redundancy metric view. Token-efficiency / tool-call-redundancy metrics over l0_session_metrics.'
dimensions:
- name: Model
  expr: model
...
- name: Total Tool Calls
  expr: SUM(total_tool_calls)
  comment: Total tool calls across traces.
- name: Tool Calls per Trace
  expr: SUM(total_tool_calls) / NULLIF(COUNT(1), 0)
  comment: Average tool calls per trace.
...
- name: Estimated Cost (USD)
  expr: SUM(est_cost_usd) FILTER (WHERE cost_priced)
  comment: Estimated USD cost over priced traces only (unpriced traces excluded).
$$
