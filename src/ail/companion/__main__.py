...
def _executor_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    if not args.warehouse_id:
        raise SystemExit("--warehouse-id is required (or set AIL_WAREHOUSE_ID)")
    if not args.volume_root:
        raise SystemExit("--volume-root is required (or set AIL_SNAPSHOT_VOLUME)")
...
def _planner_args(args: argparse.Namespace) -> argparse.Namespace:
    ...
    return argparse.Namespace(
        ...
        goal_target_kind=args.goal_target_kind,
        guardrail_judge=args.guardrail_judge,
...
def run_poll(argv: list[str]) -> int:
    ...
    exec_args = _executor_args(args)
    plan_args = _planner_args(args) if args.plan_every else None
    _auth(exec_args)
