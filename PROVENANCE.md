...
| `src/ail/registry.py`, `config/agents.yaml`, `tests/test_registry.py` | Agent registry: typed `Agent`/`AgentRegistry` (pydantic v2, `extra="forbid"`) mapping `agent_name → experiment_id (+ judge/tag config)`, with a YAML loader and an in-code seed. Consumes only `ail.cohorts` + pydantic/pyyaml. **Original (clean-room).** |
| `src/ail/publish_versions.py`, `tests/test_publish_versions.py` | Tier-A publish extension: aggregates a Phase-2 `Phase2Artifact`'s already-computed L0 values to the **per-(agent, version)** level and writes the unified `agent_registry`/`agent_version_l0`/`agent_version_comparison`/`agent_version_readiness` UC tables. Reuses `ail.publish` (atomic staging→`REPLACE WHERE` swap), `ail.compare`/`ail.optimize.phase2` contracts, and wires `ail.readiness.compute_readiness` to gate a Python-decided display status. Recomputes no metric. **Original (clean-room).** |
